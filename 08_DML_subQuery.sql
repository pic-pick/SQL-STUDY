...
-- 비교연산자는 한 개의 값만 비교할 수 있음
-- 주문한 고객의 고객번호는 여러 값이 반환됨(여러 행이 반환) -> 비교 연산자 사용 불가
