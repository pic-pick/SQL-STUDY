...
-- 하위 질의 수행 후 반환 릴레이션에 대하 다름 질의 릴레이션으로 포함시킴
...
-- 서크쿼리 질의 결과값이 다중행인 경우 : 다중행 서브쿼리 (IN, ANY, ALL, EXISTS 연산자 사용)
...
-- 2. BOOKNO 1004번의 도서가 주문된 적이 있다면 주문한 고객의 번호를 조회
SELECT CLIENTNO
FROM BOOKSALE
WHERE BOOKNO='1004';
...
-- 단일행 서브쿼리의 조건 연산으로 in tkdydgoeh anqkdgka
SELECT CLIENTNAME
FROM CLIENT
...
-------------------------------------------------------------------------------
-- 다중행 서브쿼리 연산자 (EXISIS, NOT EXISIS)
-- EXISIS : 서브쿼리의 결과가 행을 반환하면 참이되는 연산자
-- 도서를 구매한 한적이 있는 고객
