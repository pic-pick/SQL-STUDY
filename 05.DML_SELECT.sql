...
-- bsDate가 null을 허용하거나 값이 중복되는 컬럼이라면 count가 원하는 목적과같이 반환되지 않을 수도 있음
...
-- 각 도서번호별 판매수량 확인
-- groupby 진행 한 경우 select 절에 집계함수를 통해 필요열의 집계진행 가능, group by에 기준되는 열은 select에 포함시킬 수 있다
...
from booksale
group by bookno
order by 2; -- select된 첫번째 열을 기준으로 정렬
...
-- groupby의 기준으로 사용하지 않은 필드는 select에 단독 사용 불가
